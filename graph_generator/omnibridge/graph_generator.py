...
class OmnibridgeGraphGenerator(BaseGraphGenerator):
    ...
    def get_router_event_list(self, blockchain):
        if blockchain == "ethereum":
            return (
                "event TokensBridgingInitiated(address token, address sender, uint256 value, bytes32 messageId), "
    ...
    def parse_bridge_router_event(self, tx, event, event_index, routing_node, graph_obj: GraphObject):
        ...
        ):  # AffirmationCompleted (sender, executor, messageId, status)
        ...
        ):  # RelayedMessage (sender, executor, messageId, status)
        ...
        ):  # RelayedMessage (recipient, value, nonce)
    ...
    def parse_user_request_for_affirmation(self, tx, event, event_index, routing_node, graph_obj: GraphObject):
        ...
        log_event_node = graph_obj.create_log_node(
            ...
            event_signature,
            {"message_id": event_record.message_id, "encoded_data": event_record.encoded_data},
            event["data"],
    ...
    def parse_tokens_bridging_initiated(self, tx, event, event_index, routing_node, graph_obj: GraphObject):
        ...
        log_event_node = graph_obj.create_log_node(
            ...
            event_signature,
            {"token": event_record.token, "sender": event_record.sender, "amount": int(event_record.value), "message_id": event_record.message_id},
            None,
            amount=value,
    ...
    def parse_tokens_bridged(self, tx, event, event_index, routing_node, graph_obj: GraphObject):
        ...
        log_event_node = graph_obj.create_log_node(
            ...
            event_signature,
            {"token": event_record.token, "recipient": event_record.recipient, "amount": int(event_record.value), "message_id": event_record.message_id},
            None,
    ...
    def parse_affirmation_completed(self, tx, event, event_index, routing_node, graph_obj: GraphObject):
        ...
        log_event_node = graph_obj.create_log_node(
            ...
            event_signature,
            {"sender": event_record.sender, "executor": event_record.executor, "message_id": event_record.message_id},
            event["data"],
            timestamp=tx.timestamp
        )
        graph_obj.create_edge(routing_node.node_id, log_event_node.node_id, GraphEdgeType.LOG_RELATION.value, event_index)
    ...
    def parse_relayed_message_recipient(self, tx, event, event_index, routing_node, graph_obj: GraphObject):
        ...
        log_event_node = graph_obj.create_log_node(
            ...
            event_signature,
            {"token": dai_token_address, "recipient": event_record.recipient, "amount": int(event_record.value), "nonce": event_record.nonce},
            event["data"],
            amount=int(event_record.value),
            amount_usd=amount_usd,
            token_symbol=token_metadata.symbol if token_metadata else None,
